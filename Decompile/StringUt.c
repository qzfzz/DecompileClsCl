...
char* trimStr( char *pStrSrc, const char *pStrMatch )
{
	...
	char *pBuf = (char*)calloc( sizeof( char ), strlen( pStrSrc ) );
	
	char *pTmp = strtok( pStrSrc, pStrMatch );
	
	while( pTmp )
	{
		strcat( pBuf, pTmp );
		
		pTmp = strtok( NULL, pStrMatch );
	}
